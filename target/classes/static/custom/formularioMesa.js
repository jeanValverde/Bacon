...
function validatorForms(formulario) {
    ...
    var validar = function (e) {
        for (var i = 0, max = formulario.elements.length; i < max; i++) {
            //validar number
            if (formulario.elements[i].id == "numeroMesa") {
                document.getElementById("mensajeNumeroMesa1").setAttribute("style", "display: display");
                if (validarSiNumero(formulario.elements[i])) {
                    var numeroMesa = new Number(formulario.elements[i].value);
                    document.getElementById("mensajeNumeroMesa2").setAttribute("style", "display: display");
                    formulario.elements[i].setAttribute("class", "form-control is-valid");
                    if (numeroMesa < 0) {
                        document.getElementById("mensajeNumeroMesa2").setAttribute("style", "display: block");
                        formulario.elements[i].setAttribute("class", "form-control is-invalid");
                        e.preventDefault(e);
                    }
                } else {
                    formulario.elements[i].setAttribute("class", "form-control is-invalid");
                    document.getElementById("mensajeNumeroMesa1").setAttribute("style", "display: block");
                    e.preventDefault(e);
                }
            }
            if (formulario.elements[i].id == "cantidadAsientosMesa") {
                document.getElementById("mensajeCantidadAsientosMesa1").setAttribute("style", "display: display");
                if (validarSiNumero(formulario.elements[i])) {
                    var cantidadAsientosMesa = new Number(formulario.elements[i].value);
                    document.getElementById("mensajeCantidadAsientosMesa2").setAttribute("style", "display: display");
                    formulario.elements[i].setAttribute("class", "form-control is-valid");
                    if (cantidadAsientosMesa < 0) {
                        document.getElementById("mensajeCantidadAsientosMesa2").setAttribute("style", "display: block");
                        formulario.elements[i].setAttribute("class", "form-control is-invalid");
                        e.preventDefault(e);
                    }
                } else {
                    formulario.elements[i].setAttribute("class", "form-control is-invalid");
                    document.getElementById("mensajeCantidadAsientosMesa1").setAttribute("style", "display: block");
                    e.preventDefault(e);
                }
            }
            if (formulario.elements[i].id == "estadoMesa") {
                if (validarSelectOne(i)) {
                    document.getElementById("mensajeEstadoMesa1").setAttribute("style", "display: display");
                    formulario.elements[i].setAttribute("class", "custom-select is-valid");
                } else {
                    document.getElementById("mensajeEstadoMesa1").setAttribute("style", "display: block");
